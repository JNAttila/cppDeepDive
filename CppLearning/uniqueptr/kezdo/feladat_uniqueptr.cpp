...
int main() {
  ...
  Rajz rajz;
  {
    const unique_ptr<Teglalap> teglalap = make_unique<Teglalap>(3, 4);
    const unique_ptr<Negyzet> negyzet = make_unique<Negyzet>(5);
    const unique_ptr<Haromszog> haromszog = make_unique<Haromszog>(3, 4, 5);
...

  {
    ...
    std::cout << "rajz.dump(): " << std::endl
              << out3 << std::endl;
    assert(out3 == "A teglalap kerulete 18 egyseg.\nA negyzet kerulete 24 egyseg.\nA haromszog kerulete 15 egyseg.\n");
